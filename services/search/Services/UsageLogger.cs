...
// schedule. A Postgres hiccup (or this channel being full) must never
// turn into extra latency or a failure on the real request path (the
// search response the mechanic is waiting on) - see docs/log-dashboard.md
// section 3. Losing a handful of usage rows if the process dies between
// TryWrite and the next flush is an accepted, deliberate tradeoff for a
// cost dashboard, not a billing system of record.
